...
import {
  ...
  listaI,
  plan_estrategico,
} from '../../Interface/planEstrategico';
...
export class PlanComponent implements OnInit {
  ...
  constructor(
    ...
    private swArchivos: SwArchivoPlanService
  ) {}
  ...
  async listarPlanes() {
    ...
    const datos: listaI = await new Promise<listaI>((resolve) =>
      this.swPlan.ListaPlanesCompletos(dat).subscribe((translated) => {
        resolve(translated);
      })
    );
    console.log(datos);
  ...
  async guardarPlan() {
    if (
      ...
      if (this.txtCodigo === 0 && !this.eliminar) {
        ...
        if (
          (moment(an1).format() <= moment(this.txtFechaI).format() &&
            moment(an2).format() >= moment(this.txtFechaF).format() &&
            this.txtAlineacion > 0) ||
          this.txtAlineacion == 0
        ) {
        ...
        const validacion = await new Promise<any>((resolve) =>
          this.swPlan.ValidarPlanes(val).subscribe((translated) => {
            resolve(translated);
          })
        );
        if (validacion.data) {
  ...
  modificarPlan(plan: plan_estrategico) {
    if (this.txtModificar) {
      ...
      this.txtFechaI =
        fechai.getDate() + '/' + mes + '/' + fechai.getFullYear();
      var fechaf = new Date(plan.plan_fecha_fin);
      var mesf = fechaf.getMonth() + 1;
      this.txtFechaF =
        fechaf.getDate() + '/' + mesf + '/' + fechaf.getFullYear();
  ...
  async OnDriveAlmacenar() {
    ...
    const datosToken = await new Promise<any>((resolve) =>
      this.swArchivos.ObtenerTokenArchivos(dat).subscribe((translated) => {
        resolve(translated);
      })
    );
    ...
    if (event.target.files.length > 0) {
      if (event.target.files[0].size > MAXIMO_TAMANIO_BYTES) {
        ...
        this.messageService.add({
          ...
          summary: this.mensajesg.CabeceraError,
          detail:
            'El tamaño de archivo permitido es de ' +
  ...
  async serviciobuffer(token: any) {
    let ruta =
      this.datosCorreo.sistema +
      '/' +
      this.datosCorreo.planificacionEstrategica +
      '/' +
      this.txtDependencia +
      '/' +
      this.txtCodigo +
      '-' +
      this.txtArchivo.name;
  ...
  async subirarchivo(archivo: any, ruta: any) {
    ...
    const datoar = await new Promise<any>((resolve) =>
      this.swArchivos.IngresarArchivo(datoa).subscribe((translated) => {
        resolve(translated);
      })
    );
    if (datoar.success) {
      ...
      this.swArchivos.verArchivos(datosToken.token, dato).subscribe((data) => {
        if (data.success) {
          this.datosCorreo.toDataURL(data.download, function (dataUrl: any) {
            if (dataUrl == '') {
              $('#ver-pdf').html(
                'No se pudo cargar el PDF, vuelva a intentar mas tarde.'
              );
            } else {
              $('#ver-pdf').html(
                ' <embed src="' +
