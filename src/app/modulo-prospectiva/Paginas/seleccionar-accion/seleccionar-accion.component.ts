...
export class SeleccionrAccionComponent implements OnInit {
  ...
  constructor(
    ...
    private swCriterio: SwCriteriosService,
    private mensajesg: MensajesGenerales,
    private messageService: MessageService,
    private swCritDes: SwCriteriosDesService,
    private swAuditoria: SwAuditoriaService,
    private swEval: SwEvalAccionService,
    private swEmail :SwCorreoService
  ) {}
  ...
  async listarCriterios() {
    ...
    const datos: listaI = await new Promise<listaI>((resolve) =>
      this.swCriterio.ListarCriteriosActivos(dat).subscribe((translated) => {
        resolve(translated);
      })
    );
    if (datos.success) {
  ...
  async listarCriteriosDes() {
    ...
    const datos: listaI = await new Promise<listaI>((resolve) =>
      this.swCritDes.ListarCriteriosDes(valores).subscribe((translated) => {
        resolve(translated);
      })
    );
    if (datos.success) {
  ...
  GuardarSeleccion(tipo: number) {
    ...
    if (tipo == 1) {
      this.tituloModal =
        '¿Está seguro que desea guardar estas acciones seleccionadas?';
      ...
      this.tituloModal =
        '¿Está seguro que desea eliminar estas acciones seleccionadas?';
    ...
    if (this.selectAccionA.length > 0) {
      for (let accion of this.selectAccionA) {
        ...
        const datos = await new Promise<any>((resolve) =>
          this.swCritDes.SeleccionarAccion(dat).subscribe((translated) => {
            resolve(translated);
          })
        );
        if (datos.success) {
          const datosAudi = {
            ...
            aud_descripcion:
              proceso +
              ' de selección de acciones: {Código:' +
              accion +
              ', estado: ' +
              estado +
              '}',
      ...
      this.messageService.add({
        ...
        detail: 'Se seleccionaron ' + suma + ' acciones',
      });
      this.modalSeleccion = false;
      this.listarCriterios();
      this.listarCriteriosDes();
      ...
      this.messageService.add({
        ...
        detail: this.mensajesg.NoSeleccionado + ' acción',
      });
    }
  }
  ...
  async GuardarTiempo() {
    if (this.txtIngresar) {
      if (this.txtProspectiva != 0) {
        ...
        const datosR: listaI = await new Promise<listaI>((resolve) =>
          this.swEval.ListarRespuesta(dat).subscribe((translate) => {
            resolve(translate);
          })
        );
        if (datosR.success && datosR.data.length > 0) {
          const datos: listaI = await new Promise<listaI>((resolve) =>
            this.swEval.ListarTiempo(dat).subscribe((translated) => {
              resolve(translated);
            })
          );
          if (datos.success) {
            if (datos.data.length > 0) {
              for (let valores of datos.data) {
                if (valores.tiem_tipo == 1) {
                  ...
                  this.txtFechaI = moment(valores.tiem_fecha_inicio).format(
                    'YYYY-MM-DDTkk:mm'
                  );
                  this.txtFechaI2=valores.tiem_fecha_inicio;
                  this.txtFechaF = moment(valores.tiem_fecha_fin).format(
                    'YYYY-MM-DDTkk:mm'
                  );
                  this.txtFechaF2=valores.tiem_fecha_fin;
                }
              }
            } else {
              this.txtNombre = '';
          ...
          this.messageService.add({
            ...
            summary: this.mensajesg.CabeceraError,
            detail:
              'Debe ingresar las respuestas en configuración de prospectiva',
          });
        }
      } else {
        this.messageService.add({
  ...
  async ingresarTiempo() {
    ...
    if(this.txtFechaF>this.txtFechaI){
      if (this.txtCodigoC == 0) {
        const datos = await new Promise<any>((resolve) =>
          this.swEval.IngresarTiempo(dat).subscribe((translated) => {
            resolve(translated);
          })
        );
        if (datos.success) {
          const datosAudi = {
            ...
            aud_descripcion:
              'Ingresar tiempo con los datos: {Nombre:' +
              this.txtNombre +
              ', Fecha inicio: ' +
              this.txtFechaI +
              ', Fecha fin: ' +
          ...
          const email={
            enem_dependencia:this.sessionDepC,
            enem_rol:6,
            enem_observacion:"se activo las fechas para realizar la encuesta, desde <b>"+ moment(this.txtFechaI2).format('YYYY MM DD, hh:mm a')+"</b> hasta <b>"+moment(this.txtFechaF2).format('YYYY MM DD, hh:mm a')+'</b> en la dependencia: <b>'+this.sesionDep+'</b>',
            enem_modulo:"Prospectiva",
            enem_tipo:"Encuesta"
          }
          const datosemail=await new Promise<any>((resolve)=> this.swEmail.IngresarEnviarE(email).subscribe((translated)=>{resolve(translated)}));
          this.modalTiempos = false;
          this.messageService.add({
            severity: 'success',
            summary: this.mensajesg.CabeceraExitoso,
        ...
        const datos = await new Promise<any>((resolve) =>
          this.swEval.ModificarTiempo(dat).subscribe((translated) => {
            resolve(translated);
          })
        );
        if (datos.success) {
          if (datos.success) {
            const datosAudi = {
              ...
              aud_descripcion:
                'Modificar tiempo con los datos: {Código: ' +
                this.txtCodigoC +
                ' ,Nombre:' +
                this.txtNombre +
                ', Fecha inicio: ' +
      ...
      this.messageService.add({
        severity: 'error',
        summary: this.mensajesg.CabeceraError,
        detail: this.mensajesg.ErrorFechas,
      });
    }
  }
}
